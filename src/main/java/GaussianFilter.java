...
// This filter currently works, or at least I can see the effects
public class GaussianFilter extends ConvolveFilter {
    ...
    public BufferedImage filter( BufferedImage src, BufferedImage dst ) {
        ...
        if ( dst == null )
            dst = createCompatibleDestImage( src, null ); // creates a zero destination image 

        int[] inPixels = new int[width*height]; // an array of int inPixels equals width times height
        int[] outPixels = new int[width*height]; // an array of int outPixels equals width times height
        src.getRGB( 0, 0, width, height, inPixels, 0, width );
        dst.getRGB( 0, 0, width, height, outPixels, 0, width );
